...
# ── Load model ────────────────────────────────────────────────────────────────
MODEL_PATH = "mlb_award_models.pkl"
...
def predict_player(player_type, stats_dict, prior_ab=0, prior_ip=0):
    ...
    if not MODEL_LOADED:
        # Demo mode: heuristic scores when no model file uploaded
        ops   = stats_dict.get('bat_OPS', 0)
        war   = stats_dict.get('bat_WAR', stats_dict.get('pit_WAR', 3))
        era   = stats_dict.get('pit_ERA_calc', 4.5)
        whip  = stats_dict.get('pit_WHIP', 1.4)
        so    = stats_dict.get('pit_SO', 0)
        hr    = stats_dict.get('bat_HR', 0)
        rbi   = stats_dict.get('bat_RBI', 0)

        mvp_s  = min(max((ops - 0.7) * 1.8 + (hr - 20) * 0.008 + (rbi - 60) * 0.004, 0), 1)
        cy_s   = min(max((4.5 - era) * 0.15 + (1.5 - whip) * 0.3 + so * 0.002, 0), 1) if player_type != 'hitter' else 0
        roy_s  = min(max(mvp_s * 0.7, 0), 1) if roy_eligible else 0

        results['MVP']     = {'score': round(mvp_s, 3), 'p_votes': min(mvp_s * 1.3, 1.0)}
        results['CyYoung'] = {'score': round(cy_s, 3),  'p_votes': min(cy_s * 1.3, 1.0)}
        results['ROY']     = {'score': round(roy_s, 3), 'p_votes': min(roy_s * 1.3, 1.0) if roy_eligible else 0}
        return results
...
with st.sidebar:
    ...
    if not MODEL_LOADED:
        st.warning("**Model not loaded.** Upload `mlb_award_models.pkl` to enable full predictions.")
        uploaded = st.file_uploader("Upload model file", type=["pkl"])
        if uploaded:
            with open(MODEL_PATH, "wb") as f:
                f.write(uploaded.read())
            st.success("Model loaded! Refresh the page.")
            st.rerun()
        st.info("Running in **demo mode** — predictions use a heuristic approximation.")
    else:
...
""")
                st.caption("Thresholds based on historical vote share distributions. "
                           "Test-set Spearman: MVP 0.29, CY 0.26, ROY 0.27 (2022–2024).")
